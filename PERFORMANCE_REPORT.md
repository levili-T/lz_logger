...
3. **智能文件切换**
   ...
   - 原子指针切换
   - 双检锁减少切换竞争
   - **性能影响分析**: 多线程时文件切换频率是单线程的 5 倍，这是性能下降的主因

4. **高效加密实现**
   - AES-256-CBC 硬件加速
   - 流式加密避免额外缓冲
   - **加密开销**: +437%（但绝对性能仍达 169 万条/秒）
...
uint32_t offset = ctx->current_offset;
ctx->current_offset += size;
pthread_mutex_unlock(&ctx->mutex);
memcpy(buffer + offset, data, size);
// 问题：即使数据拷贝在锁外，仍有锁竞争开销

// LZ Logger 的真正无锁设计（CAS 原子操作）
while (true) {
    ...
    if (atomic_compare_exchange_weak(ctx->cur_offset_ptr, 
                                      &current_offset, 
                                      new_offset)) {
        // CAS 成功，预留的空间完全属于当前线程
        void *write_ptr = (uint8_t *)ctx->mmap_ptr + current_offset;
        memcpy(write_ptr, message, len);  // 完全无锁写入
        break;
    }
    // CAS 失败，重试（乐观锁）
}
...
1. **文件切换频率增加**（主要原因）
   - 单线程：100,000 条日志切换 ~2 次文件
   - 5 线程：100,000 条日志切换 ~10 次文件（5 倍）
   - 文件切换需要加锁 + 创建新文件 + mmap（~100-200 微秒）
   - **影响**: 约占总耗时的 50-60%

2. **CPU 缓存竞争**
   - 多线程共享 `cur_offset_ptr`，导致缓存行失效
   - **影响**: 约占总耗时的 10-15%
...
> **结论**: 真正的无锁设计已经做到极致，性能下降主要是文件切换和硬件限制导致的。
...
| 文件切换开销 | ~50% | **主要瓶颈**（切换频率 5 倍） |
| CPU 缓存竞争 | ~10% | CAS 竞争导致的缓存失效 |
| 其他 | ~5% | 可忽略 |

> **关键发现**: 多线程性能下降主要因为文件切换频率增加（单线程 2 次 → 多线程 10 次），而非锁竞争！
...
|------|---------|----------|
| AES 加密 | ~60% | 已使用硬件加速 |
| memcpy (数据拷贝) | ~25% | 受硬件限制 |
| 互斥锁 | ~5% | 已优化至极限 |
| 其他 | ~10% | 可忽略 |

**优化结论：**
- ✅ **已实现真正的无锁写入**（CAS 原子操作，零互斥锁）
- ⚠️ **主要瓶颈是文件切换**（多线程时切换频率增加 5 倍）
- ✅ **数据拷贝性能已接近硬件极限**（1072 MB/秒，受内存带宽限制）
- ✅ **加密已使用硬件加速**（CPU AES-NI 指令集）
- 💡 **潜在优化方向**：增大单文件大小，减少文件切换频率
