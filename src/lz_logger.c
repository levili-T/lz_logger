...
typedef struct lz_logger_context_t {
    ...
    // ============ 热路径字段（原子操作） ============
    _Atomic(atomic_uint_least32_t*) cur_offset_ptr; // 原子指针：指向当前 offset（32位CAS操作）
    _Atomic(atomic_uint_least32_t*) old_offset_ptr; // 旧文件的offset指针（延迟销毁）
    _Atomic(_Bool) is_closed;                        // 是否已关闭
} lz_logger_context_t;
...
/**
 * 从 offset_ptr 读取文件大小
 * @param offset_ptr 指向 used_size 的指针（footer最后4字节）
 * @return 文件大小
 * 
 * Footer布局（32字节，保证offset_ptr 64位对齐）：
 * [salt 16B][magic 4B][padding 4B][file_size 4B][used_size 4B]
 *  ↑                               ↑             ↑
 *  offset 0                        offset 24     offset 28 (8字节对齐)
 */
static inline uint32_t get_file_size_from_offset_ptr(atomic_uint_least32_t *offset_ptr) {
    // offset_ptr 指向 used_size（footer最后4字节，offset 28）
    // file_size 在 used_size 前面4字节（offset 24）
...
static lz_log_error_t create_and_extend_file(const char *file_path,
    ...
    do {
        ...
        // 写入padding（4字节，保证offset_ptr 64位对齐）
        uint32_t padding = 0;
        ...
        LZ_DEBUG_LOG("Wrote footer: salt + magic + padding + file_size(%u) + used_size to offset %lld", 
...
lz_log_error_t lz_logger_open(const char *log_dir,
    ...
    do {
        ...
        if (file_offset != used_size) {
            LZ_DEBUG_LOG("Sync offset: file=%u, expected=%u", 
...
static lz_log_error_t switch_to_new_file(lz_logger_context_t *ctx) {
    ...
    do {
        ...
        if (len > cached_max_data_size) {
            LZ_DEBUG_LOG("Drop log: len=%u exceeds max_data_size=%u", 
                         len, cached_max_data_size);
...
FFI_PLUGIN_EXPORT lz_log_error_t lz_logger_export_current_log(
    ...
    do {
        ...
        // 写入footer: [盐16字节][魔数4字节][padding4字节][文件大小4字节][已用大小4字节]
